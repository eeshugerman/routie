...
pub trait Draw {
    fn draw(&self, cr: &Context, network: &road::Network) -> Result<(), Error>;
}
...
impl Draw for road::Junction {
    fn draw(&self, cr: &Context, _network: &road::Network) -> Result<(), Error> {
...
impl Artist<'_> {
    ...
    pub fn draw_road_network(&self) -> Result<(), Error> {
